                ...
                <form onSubmit={handleSubmit} className="space-y-5">
                  ...
                  <div>
                    ...
                    <Textarea value={formData.need} onChange={e => setFormData({ ...formData, need: e.target.value })} required
                      placeholder={t("contact.form.needPlaceholder")} rows={5} className="bg-card border-border resize-none" />
                  </div>
              ...
              <motion.div initial={{ opacity: 0, x: 30 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} className="space-y-8">
                ...
                <div className="grid grid-cols-3 gap-3">
                  <a href="https://wa.me/225XXXXXXXX" target="_blank" rel="noopener noreferrer"
                    className="flex flex-col items-center gap-2 p-4 bg-card border border-border rounded-xl hover:border-primary/30 hover:shadow-card transition-all">
                    <MessageCircle size={24} className="text-primary" />
                    <span className="text-xs font-body text-foreground">{t("contact.whatsapp")}</span>
                  </a>
                  <a href="tel:+225XXXXXXXX"
                    className="flex flex-col items-center gap-2 p-4 bg-card border border-border rounded-xl hover:border-primary/30 hover:shadow-card transition-all">
                    <PhoneCall size={24} className="text-primary" />
                    <span className="text-xs font-body text-foreground">{t("contact.call")}</span>
                  </a>
                  <a href="mailto:contact@cypherglobal.ci"
                    className="flex flex-col items-center gap-2 p-4 bg-card border border-border rounded-xl hover:border-primary/30 hover:shadow-card transition-all">
                    <Mail size={24} className="text-primary" />
                    <span className="text-xs font-body text-foreground">{t("contact.email")}</span>
                  </a>
                </div>
              </motion.div>
            </div>
